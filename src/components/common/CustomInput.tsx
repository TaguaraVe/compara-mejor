    ...
    <div className="relative py-0  mx-auto mb-4 flex flex-col md:pt-6">
      <textarea
        ...
        className={`px-2 py-1 rounded-lg border-2 border-transparent outline-0 focus:border-2 focus:border-myBlue ${
          error?.message ? 'outline-2 outline-error border-2 border-error' : ''
        } `}
      ></textarea>
  ...
  return (
    <div className="relative mb-6 flex h-20 flex-col text-left">
      ...
      <input
        ...
        placeholder={placeholder}
        type={type}
        className="focus:border-b-6 border-b-4 border-myBlue bg-white py-2  outline-0 focus:border-myBlue px-2"
      />
      {error != null && <p className="text-sm text-error">{error.message}</p>}
    </div>
  );
